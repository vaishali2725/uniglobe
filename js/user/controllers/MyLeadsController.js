angular.module('crmUserApp').controller('MyLeadsController', function($rootScope, $scope, $http, $timeout,$interval) {
 ...
 {name: 'Zimbabwe', code: 'ZM',mcode:'+263'},
         ...
         $http.post("getMyLeads").success(function(myLeads){
		  ...
		  $scope.editlead=function(lid){
			  console.log(lid);
			$http.post("Report_Controller/get_leadbyid?id="+lid).success(function(leaddata){
				$scope.leadtoedit = leaddata; 
				$scope.st = $scope.leadtoedit[0]['status'];
				$scope.editleadform=true;
				$scope.hide_leads  = true;
			 });  
		  }
		  $scope.delete_confirmation = function(ldid){
			  $('#confModal').modal('show');
			  $scope.ldid = ldid;
		  }
		  
		  $scope.deletelead=function(lid){
			  $scope.msg = 'Lead deleted successfully';
			$http.post("Main_Controller/delete_lead?id="+lid).success(function(leaddata){
		  ...
		  $scope.closeleads=function(){
			  $scope.cleads=true;
			  $scope.pleads=true;
		  }
	
      getCloseLeads();
       	 function getCloseLeads(){  
         $http.post("getCloseLeads").success(function(myLeads){
          $scope.myLeads = myLeads;
               console.log(myLeads);
            $scope.data2 = myLeads;
            $scope.viewby = 10;
            $scope.totalItems = $scope.data2.length;
            $scope.currentPage = 1;
            $scope.itemsPerPage = $scope.viewby;
            $scope.maxSize = 5;
            });
  ...
  /* $scope.clicked=function(id){
	    $scope.id=id;
	    window.location = "#/view_leads?id="+id;
 ...
 $scope.setItemsPerPage = function(num) {	
  ...
  $scope.viewlead_action = function($id){
	 
    $http.post("Main_Controller/viewlead_action?id="+$id).success(function(data){
        $scope.data.action = data;
  ...
  $scope.viewLead = function (leadid){
   ...
   $scope.addmyaction = function (leadid){
	   $scope.action.leadid=angular.copy(leadid);
