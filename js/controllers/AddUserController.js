angular.module('crmApp').controller('AddUserController', function($rootScope, fileUpload,$scope, $http, $timeout) {
    $scope.$on('$viewContentLoaded', function() {   
        ...
        $scope.addUser = function () {
            ...
            var uploadUrl = "Main_Controller/add_profile_picture";
            var text = $scope.name;
           fileUpload.uploadFileToUrl(file, uploadUrl, text);
			
        $http.post("addUser?user="+JSON.stringify($scope.user)+"&filename="+file.name).success(function(d){
			console.log(d);
			$("#myModal").modal('show');
	   ...
	   $scope.onchange=function(c){
		   $scope.user.countrycode=angular.copy(c);
	   }
...
crmApp.directive('fileModel', ['$parse', function ($parse) {
    return {
    restrict: 'A',
...
crmApp.service('fileUpload', ['$http', function ($http) {
